...
import {createPet, deletePetApi, getPets, getPetsModifiedSince, newWebSocket, updatePet} from "./petApi";
...
const DELETE_PET_SUCCEEDED = 'DELETE_PET_SUCCEEDED';
    ...
    (state, {type, payload}) => {
        ...
        switch (type) {
            ...
            case DELETE_PET_SUCCEEDED:
                let petsBeforeDelete = [...(state.pets || [])];
                const index = petsBeforeDelete.findIndex(e => e._id === payload._id);
                if (index !== -1) {
                    // @ts-ignore
                    petsBeforeDelete.splice(index, 1);
                }
                return {...state, pets: petsBeforeDelete};
...
export const PetProvider: React.FC<ItemProviderProps> = ({children}) => {
    ...
    const deletePet = useCallback<DeleteFn>(deleteEntity, [token]);
    ...
    function sendUnsavedData() {
        ...
        async function sendUnsaved() {
            ...
            if (connected && !sendingUnsaved) {
                ...
                dispatch({type: SEND_UNSAVED_SUCCESSFUL, payload: {pets: result, sendAgain: retry}});
            }
        }
    }
        ...
        if (token?.trim() && socketClosed) {
            ...
            closeWebSocket = newWebSocket(token, async message => {
                ...
                if (event === 'created' || event === 'updated') {
                    ...
                    await Storage.remove({key: `pet${pet._id}`});
                    dispatch({type: DELETE_PET_SUCCEEDED, payload: pet});
                }
            }, () => {
                log("ws onClose");
    ...
    function deleteEntity(id: string) {
        (async () => {
            try {
                await deletePetApi(token, id);
                log("get entities from local storage");
                // @ts-ignore
                await Storage.remove({key: `pet${id}`});
                dispatch({type: DELETE_PET_SUCCEEDED, payload: {id}});
                return true;
            } catch (error) {
                log('updateEntity failed');
                return false;
            }
        })()
    }
