...
void testCreeaza() {
	...
	for (int i = 0; i < 10; i++) { //cautam in multimea vida
		assert(m.cauta(i) == false);
	}
	for (int i = 0; i < 10; i++) { //stergem din multimea vida
...
void testAdauga() {
	...
	assert(m.vida() == false);
	assert(m.dim() == 10);
	for (int i = 0; i < 30; i++) { //mai adaugam elemente [-10, 20)
		m.adauga(i);
	}
	//printM(m);
	assert(m.vida() == false);
	assert(m.dim() == 30);
	for (int i = 0; i < 200; i++) { //mai adaugam elemente [-100, 100)
		m.adauga(i);
	}
	//printM(m);
	assert(m.vida() == false);
	...
	for (int i = 19999; i >= 0; i--) { //adaugam mult, si acum prima data adaugam valori mari, dupa aceea mici
		m.adauga(i);
	}
	assert(m.dim() == 20000);
}
...
void testIterator() {
	...
	Multime m2;
	for (int i = 0; i < 200; i++) { //adaug 200 de elemente, fiecare de 3 ori
		m2.adauga(i);
		m2.adauga(i);
		m2.adauga(i);
	}
...
void testSterge() {
	Multime m;
	for (int i = 0; i < 200; i++) { //stergem din multimea vida
		assert(m.sterge(i) == false);
	}
	assert(m.dim() == 0);
	//printM(m);
	for (int i = 0; i < 200; i = i + 2) { //adaugam elemente din 2 in 2 (numere pare)
		m.adauga(i);
	}
	for (int i = 0; i < 200; i++) { //stergem tot (inclusiv elemente inexistente)
		if (i % 2 == 0) {
	...
	for (int i = 0; i <= 200; i = i + 2) { //adaugam elemente din 2 in 2
		m.adauga(i);
	}
	//printM(m);
	for (int i = 200; i > 0; i--) { //stergem descrescator (in ordine inversa fata de ordinea adaugarii)
		if (i % 2 == 0) {
			assert(m.sterge(i) == true);
		}
		else {
	...
	for (int i = 0; i < 200; i++) { //adaugam de 5 ori pe fiecare element
		m.adauga(i);
		m.adauga(i);
		m.adauga(i);
		m.adauga(i);
	...
	//printM(m);
	assert(m.dim() == 200);
	for (int i = 0; i < 400; i++) { //stergem elemente inexistente si existente
		if (i >= 200) {
			assert(m.sterge(i) == false);
		}
		else {
